...
import { validateResume, profileValidators, profileSections } from '../../../lib/candidateProfileValidation'
import { profilePath, profileSteps } from '../../../lib/candidateProfileOptions'
...
export default function ResumeUpload() {
  ...
  const [invalidStep, setInvalidStep] = useState(null)
  ...
  const submit = async (event) => {
    ...
    setError(''); setInvalidStep(null)
    const resumeErrors = validateResume(profile.resume)
    setFileError(resumeErrors.resume || '')
    if (resumeErrors.resume) return
    const index = profileValidators.slice(0, 7).findIndex((validate, i) => Object.keys(validate(profile[profileSections[i]])).length)
    if (index >= 0) { setInvalidStep(index + 1); setError('Please complete the required information before submitting your profile.'); return }
    pending.current = true
    setBusy(true)
    try {
      await candidateProfileApi.submitCandidateProfile(profile)
      completeProfile()
      toast('Your profile was submitted successfully.')
      navigate(profilePath(9), { replace: true })
    } catch (requestError) { setError(requestError.message) }
    finally { pending.current = false; setBusy(false) }
  }
  return <CandidateProfileLayout step={8} onSubmit={submit} busy={busy} error={error}>
    ...
    {profile.resume && <div className="flex min-w-0 items-center gap-3 rounded-lg border border-ink-200 p-3"><FileText className="shrink-0 text-green-600" size={24} /><div className="min-w-0 flex-1"><p className="break-all text-sm font-semibold text-ink-900">{profile.resume.name}</p><p className="mt-1 text-xs text-ink-400">{(profile.resume.size / 1024 / 1024).toFixed(2)} MB · Ready to submit</p></div><CheckCircle2 size={18} className="shrink-0 text-green-600" /><button type="button" aria-label="Remove resume" disabled={busy} className="rounded p-1 text-ink-600 hover:bg-ink-50" onClick={() => { updateSection('resume', null); setFileError('') }}><X size={18} /></button></div>}
    <p className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-700"><Info size={16} className="mt-0.5 shrink-0" />Your resume will be submitted with your profile for review. It has not been uploaded yet.</p>
    {invalidStep && <Link to={profilePath(invalidStep)} className="inline-block text-sm font-semibold text-brand-600 underline">Review {profileSteps[invalidStep - 1][1]}</Link>}
